...
export default function AIChatPanel() {
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-24 right-6 w-14 h-14 rounded-full bg-orange-600 text-white shadow-lg hover:shadow-xl hover:bg-orange-700 transition-all flex items-center justify-center z-[60] group"
  ...
  return (
    <div
      className="fixed bottom-24 right-6 w-[360px] max-w-[calc(100vw-2rem)] h-[520px] bg-white rounded-2xl shadow-2xl border border-gray-200 z-[60] flex flex-col overflow-hidden animate-fade-up"
    ...
    >
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/50">
            ...
            <div
              ...
              className={`max-w-[85%] px-3.5 py-2 text-sm leading-snug whitespace-pre-line ${
                m.role === "user"
                  ...
                  : "bg-white text-gray-800 border border-orange-100 rounded-2xl rounded-tl-sm"
              }`}
            >
              {m.text}
            </div>
        ...
        {busy && (
          <div className="flex justify-start">
            <div className="bg-white border border-orange-100 rounded-2xl rounded-tl-sm px-3.5 py-2 text-sm text-gray-500 flex gap-1">
              <span className="w-1.5 h-1.5 bg-orange-500 rounded-full pulse-dot" />
              <span className="w-1.5 h-1.5 bg-orange-500 rounded-full pulse-dot" style={{ animationDelay: "0.2s" }} />
              <span className="w-1.5 h-1.5 bg-orange-500 rounded-full pulse-dot" style={{ animationDelay: "0.4s" }} />
            </div>
          </div>
        )}
      </div>
        ...
        <div className="px-4 py-2 border-t border-gray-100 flex flex-wrap gap-1.5 bg-white">
          {QUICK_PROMPTS.map((p) => (
            <button
              ...
              onClick={() => send(p)}
              className="text-[11px] px-2.5 py-1 bg-orange-50 text-orange-700 border border-orange-100 rounded-full hover:bg-orange-100"
            >
              {p}
            </button>
          ))}
      ...
      >
        ...
        <button
          ...
          className="w-8 h-8 rounded-full bg-orange-600 text-white flex items-center justify-center hover:bg-orange-700 disabled:opacity-50"
        >
